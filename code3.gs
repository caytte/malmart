...
function doGet() {
  var html = HtmlService.createHtmlOutputFromFile('login')
    ...
    .setFaviconUrl('https://drive.google.com/uc?id=ファビコンID&.png');
  return html;
}

function processLogin(id, password) {
  // スプレッドシート2のデータを取得
  var spreadsheet2 = SpreadsheetApp.openById('スプレッドシートID');
  ...
  if (credentialsExist) {
    // スプレッドシート1のデータを取得
    var spreadsheet1 = SpreadsheetApp.openById('スプレッドシートID');
    ...
    for (var j = 0; j < data1.length; j++) {
      if (data1[j][0] === id) {
        ...
        return 'PAYMENT STATUS :SUCCESS. ' +"(" + data1[j][1] + " YEN LEFT)";
      }
    }
    return 'REQUEST DENIED (invalid ID or PASSWORD)'; // IDが存在しない場合のメッセージ
  } else {
